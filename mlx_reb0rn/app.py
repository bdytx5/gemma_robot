...
class GemmaRobotApp:
    ...
    def _build_ui(self):
        ...
        tk.Label(cfg, text="dtype", bg=BG2, fg=TEXT_DIM,
                 font=(FONT, 11)).grid(row=1, column=6, sticky="w", padx=(12, 6), pady=(8, 0))
        self._dtype_var = tk.StringVar(value=COMPUTE_DTYPE)
        dtype_cb = ttk.Combobox(cfg, textvariable=self._dtype_var,
                                 values=["float16", "bfloat16", "float32"],
                                 font=(FONT, 11), width=10, state="readonly")
        dtype_cb.grid(row=1, column=7, sticky="w", pady=(8, 0))
        ...
        self._infer_mode_cb = ttk.Combobox(cfg, textvariable=self._infer_mode_var,
                                            ...
                                            font=(FONT, 11), width=12, state="readonly")
        self._infer_mode_cb.grid(row=1, column=9, sticky="w", pady=(8, 0))

        # ---- norm row ----
        tk.Label(cfg, text="State norm", bg=BG2, fg=TEXT_DIM,
                 font=(FONT, 11)).grid(row=2, column=0, sticky="w", padx=(0, 8), pady=(8, 0))
        self._state_norm_var = tk.StringVar(value="min/max")
        ttk.Combobox(cfg, textvariable=self._state_norm_var,
        ...
        tk.Label(cfg, text="Grip denorm", bg=BG2, fg=TEXT_DIM,
                 font=(FONT, 11)).grid(row=2, column=2, sticky="w", padx=(12, 8), pady=(8, 0))
        self._grip_norm_var = tk.StringVar(value="min/max")
        ttk.Combobox(cfg, textvariable=self._grip_norm_var,
                     values=["min/max", "q01/q99"],
                     font=(FONT, 11), width=9, state="readonly").grid(
                     row=2, column=3, sticky="w", pady=(8, 0))
        self._grip_norm_var.trace_add("write", self._on_norm_change)

        tk.Label(cfg, text="Stats", bg=BG2, fg=TEXT_DIM,
                 font=(FONT, 11)).grid(row=2, column=4, sticky="w", padx=(12, 8), pady=(8, 0))
        self._stats_src_var = tk.StringVar(value="checkpoint")
        ttk.Combobox(cfg, textvariable=self._stats_src_var,
    ...
    def _eval_thread(self):
        ...
        gpu_mode   = self._infer_mode_var.get() == "GPU (server)"
